...
幸运的是，对于机器人，你有办法解决这样的问题。您的CheckSum通过引入从以前的字符计算出来的最终字符来帮助减少错误的可能性。通过适当的推理，最终的角色总是可以计算出来的。这样，当输入输入时，您可以立即验证最终字符与由CheckSum预测的字符匹配。如果两者不匹配，则输入被拒绝。最终的结果是您会看到更少的数据输入错误。

CheckSum如何工作：
CheckSum推理将需要地图点。这是一个定义查找如何处理生成最终字符所需的输入，这将使我们能够立即验证我们的输入是否正确。换句话说，对于输入的每个序列字符，我们确信只能有一个可能的输入，这要归功于这个最终字符。
...
要生成'地图点字符查找'表：
加倍角色的价值。
如果这种倍增操作的结果大于9（例如：7 * 2 = 14），则通过将产品数字相加来减少它。 （即找到它的数字根，例如，10：1 + 0 = 1,14：1 + 4 = 5）只做一次。即使总和大于9，也不要管它。
你能举个例子吗？
...
字母数字输入
为了使这更有趣，我们可以使用字母数字输入，这是一个可能的10位数字和26个大写字母的组合。 这意味着我们必须将我们的地图点升级为支持信件。 我们如何做到这一点？ 我们使用每个字符的ASCII值来帮助我们确定字符序列。 例如：'A'的ASCII值为65.为了确定它在地图上的顺序，我们需要减去48。

对于这个例子，'A'是7，因为65 - 48 = 17，17 * 2 = 34和3 + 4 = 7（记住只应用'地图点字符查找'到偶数索引字符。 一个奇怪的索引字符，它的值是17）
字母数字例子
