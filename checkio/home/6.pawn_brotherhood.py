...
世界上几乎每个人都知道古老的象棋，至少对它有一个基本的了解
规则。它有各种各样的单位，有各种各样的运动模式，允许有大量的可能
不同的游戏位置(例如在n个plies结尾的可能的棋类游戏)。对于这个
我们将考察棋子的运动和行为。

国际象棋是一种双人策略游戏，在棋盘上排列的棋盘上有八行(称为等级)
用数字1到8)和8个列(称为文件，用字母a表示h)。
棋盘的每一个方格都由一个唯一的坐标对来标识——一个字母和一个数字(ex，“a1”，“h8”，
“d6”)。对于这一任务，我们只需要关注卒。棋子可以捕捉对手的棋子
一个正方形对角线在它前面的一个相邻的文件，通过移动到那个正方形。白色的棋子在前面
正方形比它们的行更大。
