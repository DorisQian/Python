...
有时人类会造出奇怪的东西。我们的机器人已经发现并希望使用一个古老圆形大炮装载系统。这个系统看起来像编号管道
以循环方式排列。这些管子后面有一个旋转机构，大炮连接到最后。这个系统是非常古老的和一些
大炮被打破了。加载自动机有一个管道编号的程序这表明它应该在哪里放置炮弹。这些数字不能改变
因为它们被刻在管道中。但是，我们可以旋转后端机制改变管道和大炮之间的对应关系。我们应该找到每个组合
我们可以旋转后端机制，以便加载所有加载的炮弹进入仍在运作的大炮。加载自动机将同时加载所有的球。

管道编号从0到N-1。后端机制的初始位置被表示为1和/或0的数组。每个元素描述了后面的一个炮
管;第0个元素描述第0个管道。 1是一个工作的大炮，0是一个破的大炮。

你知道自动机将加载炮弹的管道号码（有时会加载
几个炮弹变成一个大炮）。你的目标是找到你所有的组合
可以顺时针旋转后端机构，使所有的炮弹将被装入工作大炮。旋转被描述为一个整数 - 多少个
你应该顺时针旋转单位。结果应该被表示为一个整数列表（变体）按升序排列。描述了不需要旋转的情况
作为0（但不要忘记其他变体）。如果找不到解决方案，则返回[]。

例如，初始状态是[1,0,0,0,1,1,0,1,0,0,0,1]，管道编号是[0,1]。
如果您将机构旋转1或8个单位，则将所有球放置在0号第一管将在大炮。
