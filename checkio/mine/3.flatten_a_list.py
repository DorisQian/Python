...
作为他努力的结果，一串简单的数字变成了一个深深的嵌套
名单。 索菲亚（Sophia）和斯蒂芬（Stephan）并不真正了解他的组织，需要指出
这是什么意思。 他们需要你的帮助来理解尼古拉斯疯狂的名单。

有一个包含整数或其他可能包含的嵌套列表的列表
更多的列表和整数，然后...你明白了。 你应该把所有的
整数值放入一个平面列表中。 顺序应该和原来一样
列表中的字符串表示从左到右。

我们需要把这个程序从尼古拉隐藏起来，保持它小而易于隐藏。
因此，您的代码应该少于140个字符（使用空格）。
