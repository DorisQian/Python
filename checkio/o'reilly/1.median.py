...
elki
中位数是一个数字值，它将上半部分和下半部分分开。在有奇数个实体的列表中，中位数是数组中间的数字。如果数组包含偶数个实体，那么就没有一个中间值，而是中间的两个数字的平均值。对于这个任务，你会得到一个非空的自然数数组(X)，你必须把上面一半的数字和下半部分分开，然后找到中位数。
输入:作为整数列表的数组。
输出:作为浮点数或整数的中位数。

如何使用:中位数用于统计和概率论，对偏态分布具有特别重要的意义。例如:我们想从一组数据中了解人们的平均财富——每个月100个人赚100美元，10个人赚100万美元。如果我们把它平均起来，我们得到91000美元。这是一个奇怪的值，并没有给我们展示真实的图片。在这种情况下，中值会给我们更有用的价值和更好的图片。在维基百科文章。
"""
def checkio(data):
