...
“是的，索菲亚，这是什么?”
“你告诉我!你的折磨设备用它的新词汇来唱我的电路。我不可能记住这些新的密码，而且这个东西不接受简单易行的密码!”
“哦，这些密码都不错，你可以用。”
“你为什么不能用呢?”索菲亚几乎歇斯底里地问道。“我已经忘记他们!你想让我永远锁在我自己的房子里吗?想出一些更简单的方法，这样我就不用把那些随机的官样文章放在脑子里了。索菲亚很少表现得如此苛刻，但她已经达到了她的沸点。过去几天的压力加上她的疲惫，使她感到紧张不安。
“别担心，我想要更容易记住的东西。”你为什么不使用密码地图来帮助你的密码。用它我们可以加密所有的密码，然后把它们放在门旁边或者我们都同意的特殊地方。你只能用我们在旅途中随身携带的密码地图来阅读。
“所以我不需要记住所有的密码?”我们所要做的就是确保我们不会丢失密码图。索菲亚满怀希望地问道。
“是的,这是正确的。”
“太棒了。给我看看密码图，再解释一下这是怎么回事。

帮助Sofia为Nikola通过密码映射加密的密码写一个解密。密码格栅是一个4×4平方的纸有四个窗户。编码器将格栅放在相同大小的纸上，将其密码的前四个符号写在窗口内(见下图)。之后，编码器顺时针旋转90度。之前写的符号被隐藏在格子里，干净的纸出现在窗户里面。然后，编码器将密码的下四个符号写入窗口，并再次将格栅旋转90度。然后，他们写下以下四个符号并再次转动格栅。最后，他们写下密码的最后四个符号。如果没有相同的密码格栅，就很难分辨出由16个符号组成的正方形的密码。因此，编码器可以确信任何流氓都不会轻易进入锁着的门。
...
如何使用:在这里您可以学习如何使用2D数组。你还可以了解古老的格栅密码，一种编码信息的技术，它已经被使用了半个世纪。对格栅密码最早的描述来自于1550年意大利数学家Girolamo Cardano。
"""
