...
在数学和数学逻辑中，布尔代数是代数的一个子领域，其中变量的值为真或假，通常表示为1或0。相反的初等代数变量的值是数字和主要的操作是加法和乘法,布尔代数的主要操作是连词(∧)表示,脱节(∨表示)和否定(¬)表示。

在这个任务中，您应该实现一些布尔操作:
——“连词”表示x∧y,满足x∧y = 1如果x = y = 1,x∧y = 0。
——“析取”表示x∨y,满足x∨y = 0如果x = y = 0,x∨y = 1。
——“暗示”(实质蕴涵)表示x→y和可以被描述为¬x y∨。如果是真的,那么x的值→y,y的。但是,如果x是假的那么y的值可以忽略;但是操作必须返回一些真值，只有两个选择，所以返回值是需要更少的值，也就是true。
——“独家”(异或)表示x⊕y和可以被描述为(x∨y)∧¬(x∧y)。它不包括两个x和y的可能性。定义在算术方面除了国防部2 1 + 1 = 0。
