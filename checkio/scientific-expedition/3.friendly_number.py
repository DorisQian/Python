...
otherwise use the standard rounding procedure. If the number of decimals is greater than
the current number of digits after dot, trail value with zeroes. The number should be a 
value with letters designating the power. You will be given a list of power designations 
(powers argument; default ['', 'k', 'M', 'G', 'T', 'P', 'E', 'Z', 'Y']). If you are given
suffix (suffix argument; default ‘’) , then you must append it. If you don’t have enough
 powers - stay at the maximum. And zero is always zero without powers, but with suffix.
...
How it is used: In the physics and IT we have a lot of various numbers. Sometimes we need to 
make them more simpler and easier to read. When you talk about gigabytes sometimes you don’t 
need to know the exact number bytes or kilobytes.
...
您应该编写一个函数，使用几个规则将一个数字转换为字符串。首先，你需要用给定的基数
来减少数字(基本参数;默认的1000)。值是小数点后的浮点数(小数参数;默认0)值,用舍入为
零法则(5.6⇒5,-5.6⇒5)如果小数点= 0,否则使用标准的舍入的过程。如果小数位数大于当前
数字后面的数字，则带0的跟踪值。数字应该是一个值，字母表示权力。你将得到一份权力分
配表(权力的论据;违约(”、“k”、“M”,‘G’,‘T’,‘P’,‘E’,' Z ',' Y '])。如果给定后缀(后缀
参数;(默认)，然后你必须附加它。如果你没有足够的能力，那就呆在最高的地方。0总是零，没有幂，但有后缀。
让我们来看看例子。这将是更简单。
...
输入:一个数字作为整数。关键字参数“base”作为一个整数，默认为1000。关键字参数“decimals”作为整数，
默认为0。关键字参数“权力”作为一个字符串列表,默认["、“k”、‘米’,‘G’,‘T’,‘P’,‘E’,' Z ',' Y ']。
输出:将转换后的数字作为字符串。

它是如何使用的:在物理学中，我们有许多不同的数字。有时我们需要让他们更简单，更容易阅读。
当你谈论字节数时，有时你不需要知道确切的字节数或千字节。
"""
import math
def friendly_number(number, base=1000, decimals=0, suffix='',
    ...
    while 1:
        if length == 1:
            flag = round(flag)
            return str(int(flag)) + pow_dic[powkey] + suffix
        if abs(flag) < base :
            if decimals == 0:
                return str(int(flag)) + pow_dic[powkey] + suffix
            else:
                return (str('%.' + str(decimals) + 'f') % flag) + pow_dic[powkey] + suffix
            break
        else:
            flag = flag / base
            powkey += 1
